...
<script>
  $( function(){
    $('#btn') .click(function(){
      ...
      $.post({
        url : '<?= base_url();?>' + 'superpanel/cms/insert',
        datatype: 'json',
        data:{'page_name':page_name, 'page_title':page_title, 'slug':slug, 'description':description, 'image':image, 
              'meta_keyword':meta_keyword, 'meta_description':meta_description, 'status':status },
        success:function(data){
          data = JSON.parse(data);
        }      
      });
